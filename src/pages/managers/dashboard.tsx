...
const Dashboard: React.FC = () => {
  ...
  const countAppointments = useApiCallBack(
    async (api, args: { branch_id: number; type: string | undefined }) =>
      await api.abys.CountAppointments(args)
  );
  ...
  const FuncCountAppointments = async () => {
    try {
      const res = await Promise.all([
        ...
        countAppointments.execute({
          branch_id: references?.branch,
          type: "done_appointments",
        }),
        countAppointments.execute({
          branch_id: references?.branch,
          type: "walkin_appointments",
        }),
      ]);
        ...
        <Container>
          ...
          >
            <Typography variant="button">
              Calendar Appointment Schedule for Branch{" "}
              {references?.branch == 1 && "Palo Alto Calamba City, Laguna"}
            </Typography>{" "}
            <br />
