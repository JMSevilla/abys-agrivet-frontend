import { Alert, Card, CardContent, Chip, Container, FormControlLabel, Grid, IconButton, Popover, Switch, Typography } from "@mui/material";
import { ControlledBackdrop, ControlledCheckbox, ControlledGrid, ControlledMultipleSelectField, ControlledSelectField, ControlledTextField, NormalButton, UncontrolledCard } from "@/components";
import { useEffect, useState } from "react";
import { useAuthenticationContext } from "@/utils/context/AuthContext/AuthContext";
import { useForm, useFieldArray } from "react-hook-form";
import { WalkedInSchema, WalkedInType } from "@/utils/schema/Appointment/WalkedInAppointmentSchema";
...
const WalkedIn: React.FC = () => {
    ...
    const checkBeforeRemoving = useApiCallBack(
        async (api, id: number) =>
        await api.abys.checkBeforeRemovingSchedule(id)
    )
    const deleteSelectedEvent = useApiCallBack(async (api, id: number) => await api.abys.handleSelectedSchedule(id))
    ...
    const createNewAppointment = useApiCallBack(
        async (api, args: CreateNewAppointment) => await api.abys.CreateNewAppointment(args)
    )
    ...
    const findAllInLobbies = useApiCallBack(
        async (api, branch_id: number) => await api.abys.FindAllWalkedInLobbies(branch_id)
    )
    const cancelAppointmentLobby = useApiCallBack(
        async (api, id: number) =>
        await api.abys.CancelAppointmentOnLobby(id)
    )
    const checkAffectedAppointments = useApiCallBack(async (api, args: {start: Date, end: Date}) => await api.abys.findAffectedSchedules(args))
    ...
    const FuncFindAllWalkedInFromLobbies = () => {
        findAllInLobbies.execute(references?.branch)
        .then((response) => {
    ...
    const {
        control, watch, trigger, resetField, setValue,
        getValues, formState : { isValid }, reset
    } = form;
    ...
    const handleCancelAppointment = (id: number) => {
        cancelAppointmentLobby.execute(id)
        .then((response) => {
    ...
    useEffect(() => {
        ...
        .then((response) => {
            const filtered = response?.data?.length > 0 && response?.data?.filter((o: any) => {
                ...
                return preFiltered.some((nested: any) => nested.branch_id == references?.branch)
            })
    ...
    const handleScheduleSelection = ({ start, end }: SlotInfo) => {
        ...
        if(eventStartDate < new Date() && !moment(start).isSame(moment(), 'day')){
            ...
            checkAffectedAppointments.execute({
                                    ...
                                    .then((res) => {
                                        if(res?.data == 200) {
                                            ...
                                            if(prompt) {
                                                ...
                                                .then((repository) => {
                                                    const id = repository?.data == null || repository?.data == "" ? lastId + 1 : repository?.data?.id + 1
                                                    const title = prompt;
    ...
    const handleSelectedEvent = (event: any) => {
        ...
        if(isPastDate && !moment(event.start).isSame(moment(), 'day')) {
            ...
            if(event.isHoliday || closedIncludes) {
                ...
                preventEventDeletionWhenSavedOnDatabase.execute(event.id)
                .then((response) => {
                    if(response.data == 200) {
                        handleOnToast(
    ...
    const lobbyColumns: any[] = [
        ...
        },
        {
            ...
            renderCell: (params: any) => {
                return  (
                    <>
                        <IconButton aria-describedby={id} onClick={handlePopOver} aria-label="more-actions">
                            <MoreVertIcon />
                        </IconButton>
                        <Popover
                        id={id}
    ...
    return (
        <>
        {loading ? <ControlledBackdrop open={loading} /> :  <Container maxWidth="xl">
                {
                    isLobby && 
                    <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 9999 }}>
                        <Alert severity="info">This walk-in appointment will proceed to waiting lobby.</Alert>
                    </div>
                }
                <UncontrolledCard>
                    <Typography variant="button">Walk-In Appointment Form</Typography> <br />
                    ...
                    <UncontrolledCard style={{ marginTop: '20px'}}>
                        <Typography variant='overline'>Drop customer preferred schedule</Typography>
                        <NormalButton 
                        sx={{
                            float: 'right',
                        }}
...
<div key={i}>
                    ...
                    <UncontrolledCard style={{ marginTop: '10px' }}>
                        <Typography variant='overline'>Notification Type Selection</Typography>
                        <hr />
                        <ControlledGrid>
                            <Grid item xs={6}>
                                <Card
              ...
              <Typography variant="caption">
                You will received an sms notification regarding your appointment
                    ...
                    handleClose={() => {setAsks(false), setBackdrop(false), setIsLobby(false)}}
                    handleDecline={() => {setAsks(false), setBackdrop(false), setIsLobby(false)}}
                    maxWidth='md'
                    title="Existing appointment detected"
                    handleSubmit={handleYesToLobby}
                    >
                        <Typography variant='button'>
                            There is an existing appointment on this day. Would the customer like to wait?
                        </Typography>
                    </ControlledModal>
                    <ControlledModal
                    open={viewLobby}
                    ...
                    >
                        ...
                        <UncontrolledCard style={{
                            ...
                            <ProjectTable pageSize={pageSize} columns={lobbyColumns} data={lobbyList} sx={{ mt: 2 }} />
                        </UncontrolledCard>
                    </ControlledModal>
                </UncontrolledCard>
            </Container>}
