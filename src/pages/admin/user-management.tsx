...
import { Chip, Container, FormControlLabel, FormLabel, Grid, Radio, RadioGroup, TextField, Typography } from "@mui/material";
...
const UserManagement: React.FC = () => {
    ...
    const globalSearch = (): Array<{id: number, firstname: string, lastname: string}> => {
        const filteredOnSearch = users.filter((value: any) => {
            return (
                ...
                || value?.id?.toString().toLowerCase().includes(searched?.toLocaleUpperCase())

            )
        })
        return filteredOnSearch
    }
    const filteredDistrubute: Array<{id: number, firstname: string, lastname: string}> | [] =
    ...
    const handleSelectUserAccess = (event : any) => {
        ...
        setPreLoad(!preload)
        if(accessLevel != 0) {
            requestToBeFilteredAccessLevel.execute(accessLevel)
            .then(response => {
                setPreLoad(false)
                    ...
                    <Typography variant='caption' gutterBottom>
                        ...
                        <UncontrolledCard style={{
                            ...
                            <ControlledGrid>
                                <Grid item xs={6}>
                                <UncontrolledCard>
                            <FormLabel id="demo-row-radio-buttons-group-label">Select user access</FormLabel>
                            <RadioGroup 
                                row
                        ...
                        <ProjectTable pageSize={5} data={
                            filteredDistrubute && filteredDistrubute
                        } columns={columns} />
                    </UncontrolledCard>
