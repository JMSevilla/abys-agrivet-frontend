...
import {
  ...
  UAMAccountType,
  uam_schema,
} from "@/utils/schema/Account/UAMSchema";
import { useAtom } from "jotai";
import {
  ProfileAtom,
  UAMAccountCreationAtom,
  UAMAccountCredentialsAtom,
          ...
          <Box className="flex items-center justify-between w-full">
            <h3 className="text-2xl font-medium font-body text-white">
              {globals?.storedType == 1
                ? "Administrator"
                : globals?.storedType == 2
                ? "Manager" +
                  " / Branch : " +
                  " " +
                  branchIdentifier(references?.branch)
                : "Customer"}
            </h3>
              ...
              <Grid item xs={6}>
                <UncontrolledCard>
                  ...
                  <NormalButton
                    variant="text"
                    size="small"
                    color={willChangePassword ? "primary" : "error"}
                    onClick={() => {
