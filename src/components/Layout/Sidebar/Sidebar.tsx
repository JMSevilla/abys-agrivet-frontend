...
const DashboardSidebar: React.FC<AdminSidebarProps> = (props) => {
    const {
      ...
      const handlePushOnParent = (uri: string, event: React.MouseEvent<HTMLDivElement, MouseEvent>, outerIndex: any) => {
        router.push(uri)
                    ...
                    <Box className="flex flex-col">
                        ...
                        <Divider className='bg-sideBarTabHover' />
                            <List style={{ marginTop: '5px' }}>
                                {sidebarConfig?.length > 0 && sidebarConfig?.map((text: any, outerIndex: any) => (
                                    <Box key={outerIndex} className='flex flex-col items-center'>
                                        ...
                                        >
                                                ...
                                                <>
                                                    {text.dropDownChildren?.length > 0 && text.dropDownChildren.map((item: any , innerIndex: any) => (
                                                        <>
                                                            ...
                                                            {item.childMenu?.length > 0 && item.childMenu?.map((child: any) => (
                                                                <>
                                                                    <Collapse
                                                                    in={item.dropDown}
                                                                    key={innerIndex}
                                            ...
                                            <>
                                                ...
                                                onClick={(event) => handlePushOnParent(text.uri, event, outerIndex)}
                            ...
                            <Divider className='bg-sideBarTabHover' />
                            <List>
                                {sidebarSettingsArea?.length > 0 && sidebarSettingsArea.map((item, index) => (
