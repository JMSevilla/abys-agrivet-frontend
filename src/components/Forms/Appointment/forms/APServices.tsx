...
import { APServicesSchema, AppointmentServiceType } from "@/utils/schema/Appointment/AppointmentSchema";
...
const ServicesForm = () => {
    ...
    const getAllBranches = useApiCallBack(api => api.abys.branchOnServicesExceptAllBranch())
    ...
    useEffect(() => {
        getAllServices.execute().then((response) => {
            ...
            const filtered = response?.data?.length > 0 && response?.data?.filter((o: any) => {
                ...
                return test.some((nested: any) => nested.branch_id == values.branch_id)
            })
