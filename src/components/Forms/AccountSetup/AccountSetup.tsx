...
import { ControlledGrid, ControlledBackdrop, UncontrolledCard, ControlledContainer, ControlledCheckbox, ControlledSelectField, NormalButton } from '@/components'
...
const AdministratorAccountSetupForm = () => {
    const {
        control, watch, resetField, trigger, getValues, setError
...
export const AdministratorAccountSetupFormAdditional = () => {
    ...
    const AccountCreationApplicationSetup = useApiCallBack(async (api, args: AccountSetup) => {
        const result = await api.users.SetupAccountRegistration(args)
        return result
    })
    const { setupCheckUsersDB } = useSetupContext()
    const { handleOnToast } = useToastContext()
    const JWTAccountSetupCreationAccount = useApiCallBack(async (api, args: JWTAccountCreationProps) => await api.authentication.JWTAccountCreation(args))
    ...
    const useApplicationRegistration = () => { 
        return useMutation((args : AccountSetup) => AccountCreationApplicationSetup.execute(args))
    }
    const useJwtAccountCreationSetup = useMutation((props: JWTAccountCreationProps) => 
